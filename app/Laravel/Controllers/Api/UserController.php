...
class UserController extends Controller{
    ...
    public function mentor(Request $request, $format = '') {
        $per_page = $request->get('per_page', 10);
        $page = $request->get('page', 1);
        // $unit = $request->get('unit_data');
        $user = $request->user();

        $this->response['msg'] = "List of Mentors";

        $users = User::types(['mentor'])->orderBy('created_at',"DESC")->paginate($per_page);

        $this->response['status'] = TRUE;
        $this->response['status_code'] = "MENTOR_LIST";
        $this->response['has_morepages'] = $users->hasMorePages();
        $this->response['data'] = $this->transformer->transform($users, new UserTransformer, 'collection');
        $this->response_code = 200;
    ...
    public function mentee(Request $request, $format = '') {
        $per_page = $request->get('per_page', 10);
        $page = $request->get('page', 1);
        // $unit = $request->get('unit_data');
        $user = $request->user();

        $this->response['msg'] = "List of Mentee";

        $users = User::types(['mentee'])->orderBy('created_at',"DESC")->paginate($per_page);

        $this->response['status'] = TRUE;
        $this->response['status_code'] = "MENTEE_LIST";
        $this->response['has_morepages'] = $users->hasMorePages();
        $this->response['data'] = $this->transformer->transform($users, new UserTransformer, 'collection');
        $this->response_code = 200;
